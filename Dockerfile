...
# (usually not needed for FastAPI apps — uv run will find ai_bi_analytics.main via PYTHONPATH implicitly)
# RUN uv pip install --no-deps --no-build .

EXPOSE 8000

# uv run ensures it uses the project's venv and finds app/ on path
CMD ["uv", "run", "uvicorn", "ai_bi_analytics.main:app", "--host", "0.0.0.0", "--port", "8000"]